...
objective.dimm.compute <- function(object, ...){
  ...
  if (object$analysis=="PCL" & object$family=="gaussian"){
    ...
    if (object$corstr=="CS"){
      ...
      func5 <- function(beta, sigma, rho, block_y, block_x, id, m, n){
        -Reduce("+", psi_g_CSderivvar_par(beta, sigma, rho, block_y, block_x, m, n))/n
      }
      d <- 2
    }
    if (object$corstr=="AR1"){
      ...
      func5 <- function(beta, sigma, rho, block_y, block_x, id, m, n){
        -Reduce("+", psi_g_AR1derivvar_par(beta, sigma, rho, block_y, block_x, m, n))/n
      }
      d <- 2
    }
    if (object$corstr=="independence"){
      ...
      func5 <- function(beta, sigma, block_y, block_x, id, m, n){
        -Reduce("+", psi_g_indderivvar_par(beta, sigma, block_y, block_x, m, n))/n
      }
      d <- 1
    }
  }
  ...
  obj$Q <- as.numeric(combined.estimation.mean(par=full.coefficients, d=d, W=object$W, 
                                               response=response, covariates=object$covariates, 
                                               J=object$J, K=object$K, N=N, div=1, func=func4, funcderiv=func5)) 
  obj$df <- (object$J*object$K-1)*(dim(object$covariates)[2]-1)
...
combined.estimation.mean <- function(par, d, W, response, covariates, J, K, N, div, func, funcderiv){
  N * psi.g.mean(par, d, response, covariates, J, K, N, func) %*% W %*% psi.g.mean(par, d, response, covariates, J, K, N, func)/div
}
...
dimm <- function(formula, data, id=id, response_indicator=NULL, subject_indicator=NULL, family, corstr, method=NULL, analysis=NULL,
  ...
  if (is.null(method)) method <- "exact"
  if (method != "exact" & method !="iterative") stop("Method must be supported type")
  if (is.null(analysis)) stop("Block analysis must be supported type")
  if (analysis != "PCL" & analysis != "GEE") stop("Block analysis must be supported type")
  ...
  if (is.null(cluster)){
    output <- dimm.compute.mean(response, covariates, response_indicator, subject_indicator, family, corstr, method, analysis, lam, folds)
  }
  if (!is.null(cluster)){
    if (cluster == 1){
      output <- dimm.compute.mean(response, covariates, response_indicator, subject_indicator, family, corstr, method, analysis, lam, folds)
      ...
      output <- dimm.compute.mean.parallel(response, covariates, response_indicator, subject_indicator, 
                                           family, corstr, method, analysis, lam, folds, cluster)
      }
  }
  ...
  if (family=="gaussian"){
    output$fitted.values <- data.frame(output$response[,which(colnames(output$response)=="id")],
                                  output$response[,-which(colnames(output$response)=="id")] - 
                                    as.matrix(output$covariates[,-which(colnames(output$covariates)=="id")]) %*% output$coefficients)
    colnames(output$fitted.values) <- c(id, "fitted.values")
  }
...
dimm.compute.mean <- function(response, covariates, response_indicator, subject_indicator, family, corstr, method, analysis, lam, folds, ...){
  time1 <- proc.time()
  output <- list()
  ...
  if (method=="exact"){
    ...
    V_psi <- output$W[1:(J*K*p), 1:(J*K*p)]
    V_g <- output$W[(J*K*p+1):(J*K*(p+d)), (J*K*p+1):(J*K*(p+d))]
    V_psig <- output$W[1:(J*K*p), (J*K*p+1):(J*K*(p+d))]
    scale <- matrix(0, p+J*K*d, p+J*K*d)
    main <- matrix(0, p+J*K*d, 1)
    for (k in 1:K){
      for(i in 1:J){
        C <- matrix(0, p+J*K*d, p+J*K*d)
        for (j in 1:J){
          C[1:p,1:p] <- C[1:p,1:p] + n_k[k]^2*
            ((
              t(S[[k]][[j]][1:p,1:p,drop=FALSE])%*%V_psi[((j-1+(k-1)*J)*p+1):((j+(k-1)*J)*p),((i-1+(k-1)*J)*p+1):((i+(k-1)*J)*p),drop=FALSE] +
                t(S[[k]][[j]][(p+1):(p+d),1:p,drop=FALSE])%*%t(V_psig)[((j-1+(k-1)*J)*d+1):((j+(k-1)*J)*d),((i-1+(k-1)*J)*p+1):((i+(k-1)*J)*p),drop=FALSE]
            ) %*% 
              S[[k]][[i]][1:p,1:p,drop=FALSE] + 
              (
                t(S[[k]][[j]][1:p,1:p,drop=FALSE])%*%V_psig[((j-1+(k-1)*J)*p+1):((j+(k-1)*J)*p),((i-1+(k-1)*J)*d+1):((i+(k-1)*J)*d),drop=FALSE] +
                  t(S[[k]][[j]][(p+1):(p+d),1:p,drop=FALSE])%*%V_g[((j-1+(k-1)*J)*d+1):((j+(k-1)*J)*d),((i-1+(k-1)*J)*d+1):((i+(k-1)*J)*d),drop=FALSE]
              ) %*% 
              S[[k]][[i]][(p+1):(p+d),1:p,drop=FALSE]
            )
          C[1:p,(d*(i-1+(k-1)*J)+p+1):(d*(i-1+(k-1)*J+1)+p)] <- C[1:p,(d*(i-1+(k-1)*J)+p+1):(d*(i-1+(k-1)*J+1)+p)] + n_k[k]^2*
            ((
              t(S[[k]][[j]][1:p,1:p,drop=FALSE])%*%V_psi[((j-1+(k-1)*J)*p+1):((j+(k-1)*J)*p),((i-1+(k-1)*J)*p+1):((i+(k-1)*J)*p),drop=FALSE] +
                t(S[[k]][[j]][(p+1):(p+d),1:p,drop=FALSE])%*%t(V_psig)[((j-1+(k-1)*J)*d+1):((j+(k-1)*J)*d),((i-1+(k-1)*J)*p+1):((i+(k-1)*J)*p),drop=FALSE]
            ) %*% 
              S[[k]][[i]][1:p,(p+1):(p+d),drop=FALSE] + 
              (
                t(S[[k]][[j]][1:p,1:p,drop=FALSE])%*%V_psig[((j-1+(k-1)*J)*p+1):((j+(k-1)*J)*p),((i-1+(k-1)*J)*d+1):((i+(k-1)*J)*d),drop=FALSE] +
                  t(S[[k]][[j]][(p+1):(p+d),1:p,drop=FALSE])%*%V_g[((j-1+(k-1)*J)*d+1):((j+(k-1)*J)*d),((i-1+(k-1)*J)*d+1):((i+(k-1)*J)*d),drop=FALSE]
              ) %*% 
              S[[k]][[i]][(p+1):(p+d),(p+1):(p+d),drop=FALSE]
            )
          C[(d*(j-1+(k-1)*J)+p+1):(d*(j-1+(k-1)*J+1)+p),1:p] <- 
            n_k[k]^2*
            ((
              t(S[[k]][[j]][1:p,(p+1):(p+d),drop=FALSE])%*%V_psi[((j-1+(k-1)*J)*p+1):((j+(k-1)*J)*p),((i-1+(k-1)*J)*p+1):((i+(k-1)*J)*p),drop=FALSE] +
                t(S[[k]][[j]][(p+1):(p+d),(p+1):(p+d),drop=FALSE]) %*% t(V_psig)[((j-1+(k-1)*J)*d+1):((j+(k-1)*J)*d),((i-1+(k-1)*J)*p+1):((i+(k-1)*J)*p),drop=FALSE]
            ) %*%
              S[[k]][[i]][1:p,1:p,drop=FALSE] +
              (
                t(S[[k]][[j]][1:p,(p+1):(p+d),drop=FALSE])%*%V_psig[((j-1+(k-1)*J)*p+1):((j+(k-1)*J)*p),((i-1+(k-1)*J)*d+1):((i+(k-1)*J)*d),drop=FALSE] +
                  t(S[[k]][[j]][(p+1):(p+d),(p+1):(p+d),drop=FALSE])%*% V_g[((j-1+(k-1)*J)*d+1):((j+(k-1)*J)*d),((i-1+(k-1)*J)*d+1):((i+(k-1)*J)*d),drop=FALSE]
              )%*%
              S[[k]][[i]][(p+1):(p+d),1:p,drop=FALSE]
            )
          C[(d*(j-1+(k-1)*J)+p+1):(d*(j-1+(k-1)*J+1)+p),(d*(i-1+(k-1)*J)+p+1):(d*(i-1+(k-1)*J+1)+p)] <- 
            n_k[k]^2*
            ((
              t(S[[k]][[j]][1:p,(p+1):(p+d),drop=FALSE])%*%V_psi[((j-1+(k-1)*J)*p+1):((j+(k-1)*J)*p),((i-1+(k-1)*J)*p+1):((i+(k-1)*J)*p),drop=FALSE] +
                t(S[[k]][[j]][(p+1):(p+d),(p+1):(p+d),drop=FALSE]) %*% t(V_psig)[((j-1+(k-1)*J)*d+1):((j+(k-1)*J)*d),((i-1+(k-1)*J)*p+1):((i+(k-1)*J)*p),drop=FALSE]
        ...
        main <- main + C[,c(1:p,(d*(i-1+(k-1)*J)+p+1):(d*(i-1+(k-1)*J+1)+p))]%*%MCLE_mean[[k]][[i]]
        scale <- scale + C
      }
    }
    output$coefficients <- as.vector(solve(scale)[1:p,]%*%main)
...
dimm.compute.mean.parallel <- function(response, covariates, response_indicator, subject_indicator, 
  ...
  print("Computing combined estimate.", quote=FALSE)
  if (method=="exact"){
    V_psi <- foreach::foreach(i = 1:(dim(psi_g)[1]), .combine="+") %dopar% {
      psi_g[i,]%o%psi_g[i,]
    }
    V_psi <- V_psi/N
    output$W <- solve(V_psi)
